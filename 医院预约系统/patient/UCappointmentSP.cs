...
namespace Hospital
{
    public partial class UCappointmentSP : UCBase
    {
        string did;
        int row, column;
        RadioButton[] rb = new RadioButton[22];
        patient pa;
        public UCappointmentSP()
        {
            InitializeComponent();
            MyInit();
        }
        public UCappointmentSP(patient pa)
        {
            InitializeComponent();
            MyInit();
            this.pa = pa;
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            string nid = GetNextNum(status).ToString();
            appointmentInfo appinfo = new appointmentInfo(pa.name, pa.id, label1.Text, label2.Text, account, time, ampm, nid);
            appinfo.ShowDialog();
            FreshRButton();
            FreshDataGridView(label1.Text);
        }
